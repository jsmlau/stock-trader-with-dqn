...
class AroonIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        rolling_close = self._close.rolling(self._window,
...
class MACD(IndicatorMixin):
    ...
    def macd(self) -> pd.Series:
        ...
        return pd.Series(macd_series,
                         name=f"MACD_{self._window_fast}_{self._window_slow}")
    ...
    def macd_diff(self) -> pd.Series:
        ...
        return pd.Series(
            macd_diff_series,
            name=f"MACD_diff_{self._window_fast}_{self._window_slow}")
...
class TRIXIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._trix = (ema3 - ema3.shift(
            1, fill_value=ema3.mean())) / ema3.shift(1, fill_value=ema3.mean())
        self._trix *= 100
...
class MassIndex(IndicatorMixin):
    ...
    Args:
        ...
        window_fast(int): fast n_days value.
        window_slow(int): slow n_days value.
        fillna(bool): if True, fill nan values.
    """
    ...
    def _run(self):
        ...
        mass = ema1 / ema2
        self._mass = mass.rolling(self._window_slow,
...
class IchimokuIndicator(IndicatorMixin):
    ...
    Args:
        high(pandas.Series): dataset 'High' column.
        low(pandas.Series): dataset 'Low' column.
        window1(int): n1 low n_days.
        window2(int): n2 medium n_days.
        window3(int): n3 high n_days.
    ...
    def _run(self):
        ...
        self._conv = 0.5 * (
            self._high.rolling(self._window1,
                               min_periods=min_periods_n1).max() +
            self._low.rolling(self._window1, min_periods=min_periods_n1).min())
        self._base = 0.5 * (
            self._high.rolling(self._window2,
                               min_periods=min_periods_n2).max() +
    ...
    def ichimoku_conversion_line(self) -> pd.Series:
        ...
        return pd.Series(conversion,
                         name=f"ichimoku_conv_{self._window1}_{self._window2}")
    ...
    def ichimoku_a(self) -> pd.Series:
        ...
        spana = (spana.shift(self._window2, fill_value=spana.mean())
                 if self._visual else spana)
        spana = self._check_fillna(spana, value=-1)
        return pd.Series(spana,
                         name=f"ichimoku_a_{self._window1}_{self._window2}")
    ...
    def ichimoku_b(self) -> pd.Series:
        ...
        Returns:
            pandas.Series: New feature generated.
        """
        spanb = 0.5 * (self._high.rolling(self._window3, min_periods=0).max() +
                       self._low.rolling(self._window3, min_periods=0).min())
        spanb = (spanb.shift(self._window2, fill_value=spanb.mean())
                 if self._visual else spanb)
        spanb = self._check_fillna(spanb, value=-1)
        return pd.Series(spanb,
                         name=f"ichimoku_b_{self._window1}_{self._window2}")
...
class KSTIndicator(IndicatorMixin):
    ...
    Args:
        close(pandas.Series): dataset 'Close' column.
        roc1(int): roc1 n_days.
        roc2(int): roc2 n_days.
        roc3(int): roc3 n_days.
        roc4(int): roc4 n_days.
        window1(int): n1 smoothed n_days.
        window2(int): n2 smoothed n_days.
        window3(int): n3 smoothed n_days.
        window4(int): n4 smoothed n_days.
        nsig(int): n n_days to signal.
        fillna(bool): if True, fill nan values.
    """
...
class DPOIndicator(IndicatorMixin):
    ...
    def _run(self):
        min_periods = 0 if self._fillna else self._window
        self._dpo = (
            self._close.shift(int((0.5 * self._window) + 1),
...
class CCIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._cci = (typical_price - typical_price.rolling(
            self._window, min_periods=min_periods).mean()) / (
                self._constant * typical_price.rolling(
                    self._window, min_periods=min_periods).apply(_mad, True))
...
class ADXIndicator(IndicatorMixin):
    ...
    def adx(self) -> pd.Series:
        ...
        for idx, value in enumerate(self._trs):
            dip[idx] = (value and 100 * (self._dip[idx] / value)) or value
        ...
        for i in range(self._window + 1, len(adx_series)):
            adx_series[i] = ((adx_series[i - 1] * (self._window - 1)) +
                             directional_index[i - 1]) / float(self._window)
    ...
    def adx_pos(self) -> pd.Series:
        ...
        adx_pos_series = self._check_fillna(pd.Series(dip,
    ...
    def adx_neg(self) -> pd.Series:
        ...
        adx_neg_series = self._check_fillna(pd.Series(din,
                                                      index=self._close.index),
                                            value=20)
...
class VortexIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._vip = vmp.rolling(self._window,
                                min_periods=min_periods).sum() / trn
        self._vin = vmm.rolling(self._window,
                                min_periods=min_periods).sum() / trn
...
class STCIndicator(IndicatorMixin):
    ...
    Args:
        ...
        smooth1(int): ema n_days over stoch_k
        smooth2(int): ema n_days over stoch_kd
        fillna(bool): if True, fill nan values.
    """
...
def ema_indicator(close, window=12, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return EMAIndicator(close=close, window=window,
...
def sma_indicator(close, window=12, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return SMAIndicator(close=close, window=window,
...
def vortex_indicator_pos(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return VortexIndicator(high=high,
                           low=low,
                           close=close,
                           window=window,
                           fillna=fillna).vortex_indicator_pos()
...
def vortex_indicator_neg(high, low, close, window=14, fillna=False):
    ...
    return VortexIndicator(high=high,
                           ...
                           close=close,
                           window=window,
                           fillna=fillna).vortex_indicator_neg()
...
def ichimoku_conversion_line(high,
                             low,
                             window1=9,
                             window2=26,
                             visual=False,
...
def aroon_down(close, window=25, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return AroonIndicator(close=close, window=window,
                          fillna=fillna).aroon_down()
...
def psar_up(high, low, close, step=0.02, max_step=0.20, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = PSARIndicator(high=high,
                              low=low,
                              close=close,
                              step=step,
                              max_step=max_step,
