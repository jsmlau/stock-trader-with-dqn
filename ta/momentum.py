...
class RSIIndicator(IndicatorMixin):
    ...
    n_days to measure speed and change of price movements of a security. It is
    ...
    def _run(self):
        ...
        emaup = up_direction.ewm(alpha=1 / self._window,
                                 min_periods=min_periods,
                                 adjust=False).mean()
        emadn = down_direction.ewm(alpha=1 / self._window,
                                   min_periods=min_periods,
                                   adjust=False).mean()
...
class TSIIndicator(IndicatorMixin):
    ...
    Args:
        close(pandas.Series): dataset 'Close' column.
        window_slow(int): high n_days.
        window_fast(int): low n_days.
        fillna(bool): if True, fill nan values.
    """
    ...
    def _run(self):
        ...
        smoothed = (diff_close.ewm(span=self._window_slow,
                                   min_periods=min_periods_r,
                                   adjust=False).mean().ewm(
                                       span=self._window_fast,
                                       min_periods=min_periods_s,
                                       adjust=False).mean())
        smoothed_abs = (abs(diff_close).ewm(span=self._window_slow,
                                            min_periods=min_periods_r,
                                            adjust=False).mean().ewm(
                                                span=self._window_fast,
                                                min_periods=min_periods_s,
...
class UltimateOscillator(IndicatorMixin):
    ...
    Average7 = (7-n_days BP Sum) / (7-n_days TR Sum)
    Average14 = (14-n_days BP Sum) / (14-n_days TR Sum)
    Average28 = (28-n_days BP Sum) / (28-n_days TR Sum)
    ...
    Args:
        ...
        close(pandas.Series): dataset 'Close' column.
        window1(int): short n_days.
        window2(int): medium n_days.
        window3(int): long n_days.
    ...
    def _run(self):
        ...
        avg_s = (
            buying_pressure.rolling(self._window1,
                                    min_periods=min_periods_s).sum() /
            true_range.rolling(self._window1, min_periods=min_periods_s).sum())
        avg_m = (
            buying_pressure.rolling(self._window2,
                                    min_periods=min_periods_m).sum() /
            true_range.rolling(self._window2, min_periods=min_periods_m).sum())
        avg_l = (buying_pressure.rolling(self._window3,
                                         min_periods=min_periods_len).sum() /
                 true_range.rolling(self._window3,
                                    min_periods=min_periods_len).sum())
        self._uo = (100.0 * ((self._weight1 * avg_s) + (self._weight2 * avg_m) +
                             (self._weight3 * avg_l)) /
...
class StochasticOscillator(IndicatorMixin):
    ...
    of a stock over a n_days of time, typically a 14-day n_days.
    ...
    Args:
        ...
        low(pandas.Series): dataset 'Low' column.
        window(int): n n_days.
        smooth_window(int): sma n_days over stoch_k.
        fillna(bool): if True, fill nan values.
    """
    ...
    def stoch_signal(self) -> pd.Series:
        ...
        stoch_d = self._stoch_k.rolling(self._smooth_window,
...
class KAMAIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        efficiency_ratio = np.divide(er_num,
                                     er_den,
                                     out=np.zeros_like(er_num),
                                     where=er_den != 0)

        smoothing_constant = ((efficiency_ratio *
                               (2.0 / (self._pow1 + 1) - 2.0 /
                                (self._pow2 + 1.0)) + 2 /
                               (self._pow2 + 1.0))**2.0).values
...
class ROCIndicator(IndicatorMixin):
    ...
    price from one n_days to the next. The ROC calculation compares the current
    ...
    def _run(self):
        self._roc = ((self._close - self._close.shift(self._window)) /
...
class AwesomeOscillatorIndicator(IndicatorMixin):
    ...
    Awesome Oscillator is a 34-n_days simple moving average, plotted through
    the central points of the bars (H+L)/2, and subtracted from the 5-n_days
    ...
    def _run(self):
        ...
        self._ao = (median_price.rolling(self._window1,
                                         min_periods=min_periods_s).mean() -
                    median_price.rolling(self._window2,
                                         min_periods=min_periods_len).mean())
...
class WilliamsRIndicator(IndicatorMixin):
    ...
    for the look-back n_days. In contrast, the Stochastic Oscillator reflects
    ...
    Lowest Low = lowest low for the look-back n_days
    Highest High = highest high for the look-back n_days
    ...
    def _run(self):
        ...
        highest_high = self._high.rolling(
            self._lbp, min_periods=min_periods).max(
            )  # highest high over lookback n_days lbp
        lowest_low = self._low.rolling(self._lbp, min_periods=min_periods).min(
        )  # lowest low over lookback n_days lbp
        self._wr = -100 * (highest_high - self._close) / (highest_high -
                                                          lowest_low)
...
class StochRSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        self._rsi = RSIIndicator(close=self._close,
                                 window=self._window,
                                 fillna=self._fillna).rsi()
...
class PercentagePriceOscillator(IndicatorMixin):
    ...
    Args:
        ...
        window_slow(int): n n_days long-term.
        window_fast(int): n n_days short-term.
        window_sign(int): n n_days to signal.
        fillna(bool): if True, fill nan values.
    """
    ...
    def ppo(self):
        ...
        return pd.Series(ppo_series,
                         name=f"PPO_{self._window_fast}_{self._window_slow}")
    ...
    def ppo_signal(self):
        ...
        return pd.Series(
            ppo_signal_series,
            name=f"PPO_sign_{self._window_fast}_{self._window_slow}")
    ...
    def ppo_hist(self):
        ...
        return pd.Series(
            ppo_hist_series,
            name=f"PPO_hist_{self._window_fast}_{self._window_slow}")
...
class PercentageVolumeOscillator(IndicatorMixin):
    ...
    def pvo(self) -> pd.Series:
        ...
        return pd.Series(pvo_series,
                         name=f"PVO_{self._window_fast}_{self._window_slow}")
    ...
    def pvo_signal(self) -> pd.Series:
        ...
        return pd.Series(
            pvo_signal_series,
            name=f"PVO_sign_{self._window_fast}_{self._window_slow}")
    ...
    def pvo_hist(self) -> pd.Series:
        ...
        return pd.Series(
            pvo_hist_series,
            name=f"PVO_hist_{self._window_fast}_{self._window_slow}")
...
def tsi(close, window_slow=25, window_fast=13, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return TSIIndicator(close=close,
...
def williams_r(high, low, close, lbp=14, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return WilliamsRIndicator(high=high,
                              low=low,
                              close=close,
                              lbp=lbp,
...
def awesome_oscillator(high,
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return AwesomeOscillatorIndicator(high=high,
                                      low=low,
                                      window1=window1,
                                      window2=window2,
...
def kama(close, window=10, pow1=2, pow2=30, fillna=False) -> pd.Series:
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return KAMAIndicator(close=close,
                         window=window,
                         pow1=pow1,
                         pow2=pow2,
                         fillna=fillna).kama()
...
def stochrsi(
    ...
    Returns:
            pandas.Series: New feature generated.
    """
    return StochRSIIndicator(close=close,
                             window=window,
                             smooth1=smooth1,
                             smooth2=smooth2,
                             fillna=fillna).stochrsi()
...
def ppo_signal(close: pd.Series,
               window_slow=26,
               window_fast=12,
               window_sign=9,
               fillna=False) -> pd.Series:
