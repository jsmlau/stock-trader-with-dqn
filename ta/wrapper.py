...
def add_volume_ta(
    ...
    # Ease of Movement
    indicator_eom = EaseOfMovementIndicator(high=df[high],
                                            low=df[low],
                                            volume=df[volume],
                                            window=14,
                                            fillna=fillna)
...
def add_trend_ta(
    ...
    if not vectorized:
        ...
        # data[f'{colprefix}trend_psar'] = indicator.psar()
...
def add_others_ta(
    ...
    """
    # Daily Return
    df[f"{colprefix}others_returns_{n_days}"] = NDaysReturnIndicator(
        close=df[close], fillna=fillna).n_days_return()

    # Daily Log Return
    df[f"{colprefix}others_dlr"] = DailyLogReturnIndicator(
        close=df[close], fillna=fillna).daily_log_return()
