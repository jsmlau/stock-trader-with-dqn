...
class BollingerBands(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._mavg = self._close.rolling(self._window,
                                         min_periods=min_periods).mean()
        self._mstd = self._close.rolling(self._window,
...
class KeltnerChannel(IndicatorMixin):
    ...
    Args:
        ...
        close(pandas.Series): dataset 'Close' column.
        window(int): n n_days.
        window_atr(int): n atr n_days. Only valid if original_version param is False.
        fillna(bool): if True, fill nan values.
    ...
    def _run(self):
        ...
        if self._original_version:
            self._tp = (((self._high + self._low + self._close) / 3.0).rolling(
                self._window, min_periods=min_periods).mean())
            self._tp_high = ((((4 * self._high) -
                               (2 * self._low) + self._close) / 3.0).rolling(
                                   self._window, min_periods=0).mean())
            self._tp_low = ((((-2 * self._high) +
                              (4 * self._low) + self._close) / 3.0).rolling(
                                  self._window, min_periods=0).mean())
        else:
            self._tp = self._close.ewm(span=self._window,
...
class DonchianChannel(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._hband = self._high.rolling(self._window,
                                         min_periods=self._min_periods).max()
        self._lband = self._low.rolling(self._window,
                                        min_periods=self._min_periods).min()
    ...
    def donchian_channel_wband(self) -> pd.Series:
        ...
        Returns:
            pandas.Series: New feature generated.
        """
        mavg = self._close.rolling(self._window,
                                   min_periods=self._min_periods).mean()
...
class UlcerIndex(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._ulcer_idx = _r_i.rolling(self._window).apply(ui_function(),
                                                           raw=True)
...
def average_true_range(high, low, close, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = AverageTrueRange(high=high,
                                 low=low,
                                 close=close,
                                 window=window,
                                 fillna=fillna)
...
def bollinger_mavg(close, window=20, fillna=False):
    ...
    N-n_days simple moving average (MA).
...
def bollinger_hband(close, window=20, window_dev=2, fillna=False):
    ...
    Upper band at K times an N-n_days standard deviation above the moving
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = BollingerBands(close=close,
                               window=window,
...
def bollinger_lband(close, window=20, window_dev=2, fillna=False):
    ...
    Lower band at K times an N-n_days standard deviation below the moving
    average (MA − Kdeviation).
...
def keltner_channel_mband(high,
                          low,
                          close,
                          window=20,
                          window_atr=10,
...
def keltner_channel_hband(high,
                          low,
                          close,
                          window=20,
                          window_atr=10,
...
def keltner_channel_lband(high,
                          low,
                          close,
                          window=20,
                          window_atr=10,
...
def keltner_channel_wband(high,
                          low,
                          close,
                          window=20,
                          window_atr=10,
...
def keltner_channel_pband(high,
                          low,
                          close,
                          window=20,
                          window_atr=10,
...
def keltner_channel_hband_indicator(high,
                                    low,
                                    close,
                                    window=20,
                                    window_atr=10,
...
def keltner_channel_lband_indicator(high,
                                    low,
                                    close,
                                    window=20,
                                    window_atr=10,
...
def donchian_channel_hband(high, low, close, window=20, offset=0, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    indicator = DonchianChannel(high=high,
                                low=low,
                                close=close,
                                window=window,
                                offset=offset,
