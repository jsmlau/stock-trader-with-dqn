...
class AccDistIndexIndicator(IndicatorMixin):
    ...
    def _run(self):
        clv = ((self._close - self._low) -
               (self._high - self._close)) / (self._high - self._low)
...
class OnBalanceVolumeIndicator(IndicatorMixin):
    ...
    def _run(self):
        obv = np.where(self._close < self._close.shift(1), -self._volume,
                       self._volume)
...
class ChaikinMoneyFlowIndicator(IndicatorMixin):
    ...
    It measures the amount of Money Flow Volume over a specific n_days.
...
class EaseOfMovementIndicator(IndicatorMixin):
    ...
    def _run(self):
        self._emv = ((self._high.diff(1) + self._low.diff(1)) *
                     (self._high - self._low) / (2 * self._volume))
        self._emv *= 100000000
...
class NegativeVolumeIndexIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._nvi = pd.Series(data=np.nan,
...
class MFIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        min_periods = 0 if self._fillna else self._window
        n_positive_mf = mfr.rolling(
            self._window, min_periods=min_periods).apply(
...
class VolumeWeightedAveragePrice(IndicatorMixin):
    ...
    intraday periods and raw_data are used in the calculation.
    ...
    def _run(self):
        ...
        min_periods = 0 if self._fillna else self._window
        total_pv = typical_price_volume.rolling(self._window,
                                                min_periods=min_periods).sum()

        # 4 total volume
        total_volume = self._volume.rolling(self._window,
                                            min_periods=min_periods).sum()
...
def on_balance_volume(close, volume, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return OnBalanceVolumeIndicator(close=close, volume=volume,
...
def chaikin_money_flow(high, low, close, volume, window=20, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return ChaikinMoneyFlowIndicator(high=high,
                                     low=low,
                                     close=close,
                                     volume=volume,
                                     window=window,
...
def force_index(close, volume, window=13, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return ForceIndexIndicator(close=close,
                               volume=volume,
                               window=window,
...
def ease_of_movement(high, low, volume, window=14, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return EaseOfMovementIndicator(high=high,
                                   low=low,
                                   volume=volume,
                                   window=window,
                                   fillna=fillna).ease_of_movement()
...
def sma_ease_of_movement(high, low, volume, window=14, fillna=False):
    ...
    return EaseOfMovementIndicator(high=high,
                                   ...
                                   volume=volume,
                                   window=window,
                                   fillna=fillna).sma_ease_of_movement()
...
def volume_price_trend(close, volume, fillna=False):
    ...
    Returns:
        pandas.Series: New feature generated.
    """
    return VolumePriceTrendIndicator(close=close, volume=volume,
...
def negative_volume_index(close, volume, fillna=False):
    ...
    n_days to the other. In other words, nothing was done. Norman Fosback, of
    ...
    """
    return NegativeVolumeIndexIndicator(close=close,
                                        volume=volume,
...
def money_flow_index(high, low, close, volume, window=14, fillna=False):
    ...
    """
    indicator = MFIIndicator(high=high,
                             low=low,
                             close=close,
                             volume=volume,
                             window=window,
...
def volume_weighted_average_price(
    ...
    indicator = VolumeWeightedAveragePrice(high=high,
                                           low=low,
                                           close=close,
                                           volume=volume,
                                           window=window,
